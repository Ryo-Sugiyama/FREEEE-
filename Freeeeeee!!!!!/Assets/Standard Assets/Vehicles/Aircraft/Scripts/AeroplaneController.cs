...
namespace UnityStandardAssets.Vehicles.Aeroplane
{
	...
	public class AeroplaneController : MonoBehaviour
	{
		...
		[SerializeField] private GameObject WeaponFirePos;
        ...
        public void FireMissile()
		{
			...
			{
				if (missiles[missileNumber].missile == null)
				{
					Destroy(missiles[missileNumber].missile = Instantiate(missileObject, WeaponFirePos.transform), 8.0f);
					missiles[missileNumber].rb = missiles[missileNumber].missile.GetComponent<Rigidbody>();
					missiles[missileNumber].missile.transform.position = WeaponFirePos.transform.position;
					//missiles[missileNumber].missile.transform.Rotate(WeaponFirePos.transform.ro);
					missiles[missileNumber].force = this.transform.forward;
					isDown = true;
				}
			}
			...
			{
				missileNumber++;
				if (missileNumber > 3) missileNumber = 0;
			}
            
			for (int i = 0; i < 4; i++)
			{
				if (missiles[i].missile == null) return;
				missiles[i].rb.AddForce(missiles[missileNumber].force * EnginePower);
